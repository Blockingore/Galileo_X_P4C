...
REM === NASCONDI/MINIMIZZA FINESTRA BAT IMMEDIATAMENTE ===
REM Prima minimizza la finestra (piu' veloce)
powershell -WindowStyle Hidden -Command "Add-Type -AssemblyName System.Windows.Forms; Add-Type -TypeDefinition 'using System; using System.Runtime.InteropServices; public class Win32 { [DllImport(\"user32.dll\")] public static extern bool ShowWindow(IntPtr hWnd, int nCmdShow); [DllImport(\"kernel32.dll\")] public static extern IntPtr GetConsoleWindow(); [DllImport(\"user32.dll\")] public static extern bool SetWindowPos(IntPtr hWnd, IntPtr hWndInsertAfter, int X, int Y, int cx, int cy, uint uFlags); }'; $hwnd = [Win32]::GetConsoleWindow(); [Win32]::ShowWindow($hwnd, 2); [Win32]::SetWindowPos($hwnd, 0, -10000, -10000, 0, 0, 0x0001)" >nul 2>&1

REM Poi prova a nasconderla completamente
...
echo $label.Text = "CONFIGURAZIONE P4C IN CORSO`n`n`nIL PROCESSO TERMINERA' AUTOMATICAMENTE`n`nATTENDERE IL COMPLETAMENTO`n`nCi vorra' circa 40 secondi..." >> "%TEMP%\block_user_interface.ps1"
